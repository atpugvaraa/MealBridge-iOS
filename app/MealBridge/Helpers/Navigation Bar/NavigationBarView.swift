...
struct NavigationBarView<Content: View>: View {
    ...
    init(title: String, blurRadius: CGFloat = 12, blurHeight: CGFloat = 60, scrollOffset: Binding<CGFloat>, icon: String? = nil, showBackButton: Bool = false, @ViewBuilder content: () -> Content) {
        self.title = title
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            .ignoresSafeArea()
            .overlay {
                NavigationBar(scrollOffset: scrollOffset, title: title, icon: icon, showTopRightButton: showTopRightButton, showBackButton: showBackButton)
            }
        }
    }
